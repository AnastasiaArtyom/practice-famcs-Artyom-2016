...
import by.bsu.up.chat.logging.impl.LogToFile;
...
public class Client {
    ...
    public void connect() {
        ...
        try {
            ...
            clientLogger.error("Could not build URL to server", e);
            throw new RuntimeException(e);
        } catch (IOException e) {
            logger.error(String.format("Could not connect to server on %s", url.toString()), e);
            clientLogger.error(String.format("Could not connect to server on %s", url.toString()), e);
        }
        ...
        Runnable listeningAction = () -> {
            ...
            while (connected && running) {
                ...
                try {
                    ...
                    clientLogger.error("The message listening thread was interrupted", e);
                    running = false;
                }
            }
        };
    ...
    private void checkConnected() {
        if (!connected) {
            ...
            clientLogger.error("No connection to server", notConnectedError);
